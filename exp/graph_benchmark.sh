...
for code in ${codes[*]};do
    for data in ${datas[*]};do
        ...
        sudo docker exec -i ${docker_name} /bin/bash -c "source /etc/profile && conda activate && cd /root/graph-benchmarks && bash run_profiler.sh code/${code} data/${data} 100 ${outfile}"
        exit
    done
done
