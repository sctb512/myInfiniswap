...
output_dir="is_iccd_memcached_lxc_multi_servers"
...
for servers_num in 1 2 4 6 8;do
    ...
    sercer_files=$(ls -lR ${output_dir} | grep ^- | grep servers_${servers_num})
    ...
    for i in $(seq 5); do
        ...
        for local in 100 90 80 70 60 50; do
            ...
            file="servers_${servers_num}_total_mem${total_mem}_local_mem${local_mem}_local${local}.txt"
            ...
            ps -ef | grep cpu_rate.sh | grep /bin/bash | awk '{print $2}' | xargs kill -s 9
            ps -ef | grep cpu_rate_lxc.sh | grep "/bin/bash" | awk '{print $2}' | xargs kill -9
            ./cpu_rate.sh "servers_${servers_num}_local_${local}_${output_dir}" ${cpu_rate_dir}/${i} &
            ./cpu_rate_lxc.sh "servers_${servers_num}_local_${local}_${output_dir}" ${docker_name} ${cpu_rate_dir}/${i} &
