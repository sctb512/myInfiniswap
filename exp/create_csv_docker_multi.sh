...
for local in 100 75 50 25;do
    ...
    sudo docker cp create_csv.py ${docker_name}:/root
    sudo docker exec -it ${docker_name} /bin/bash -c "cd /root && mkdir ${output_dir} && ls && (time python3 create_csv.py ${df_num}) 2> ${output_dir}/${file_name}"
