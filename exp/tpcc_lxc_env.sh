...
sudo lxc exec ${docker_name} -- sudo --login --user root /usr/bin/zsh -ic 'cd ~ && echo "export JAVA_HOME=$(pwd)/jdk-1.8" >> ~/.bashrc && source ~/.bashrc'
sudo lxc exec ${docker_name} -- sudo --login --user root /usr/bin/zsh -ic 'cd ~ && echo "export JRE_HOME=$(pwd)/jdk-1.8/jre" >> ~/.bashrc && source ~/.bashrc'
sudo lxc exec ${docker_name} -- sudo --login --user root /usr/bin/zsh -ic 'cd ~ && echo "export CLASSPATH=$(pwd)/jdk-1.8/lib:$(pwd)/jdk-1.8/lib:${CLASSPATH}" >> ~/.bashrc && source ~/.bashrc'
sudo lxc exec ${docker_name} -- sudo --login --user root /usr/bin/zsh -ic 'cd ~ && echo "export PATH=$(pwd)/voltdb/bin:$(pwd)/jdk-1.8/bin:$(pwd)/jdk-1.8/jre/bin:${PATH}" >> ~/.bashrc && source ~/.bashrc'
...
sudo lxc exec ${docker_name} -- sudo --login --user root /usr/bin/zsh -ic 'cd ~ && echo "export PATH=$(pwd)/voltdb/bin:${PATH}" >> ~/.bashrc && source ~/.bashrc'
