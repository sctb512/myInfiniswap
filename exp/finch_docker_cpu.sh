...
for i in `seq 10`;do
    ...
    for local in 100 95 90 85 80 75 70 65 60 55 50;do
        ...
        sudo docker exec -it ${docker_name} /bin/bash -c "cd /root && rm -rf ${output_dir} && rm -rf ${out_dir}"  >/dev/null 2>&1
        sudo docker exec -it ${docker_name} /bin/bash -c "cd /root && mkdir ${output_dir} && mkdir ${out_dir} && (time python3 /users/bin_tang/FINCH-Clustering/python/finch.py --data-path /users/bin_tang/FINCH-Clustering/data/mnist10k/data.csv --output-path ${out_dir}) 2> ${output_dir}/${file}"
